...
Este repositório contém os códigos dos exercícios e projetos que desenvolvi na plataforma da [DIO](https://web.dio.me/track/coding-the-future-xp-full-stack-developer), no bootcamp da XP de Full Stack Developer.  
...
### 4. Switch Case: Verificação de Vogais e Menu Interativo
- **Verificação de Vogais**:
  - Aplicação da estrutura `switch` para identificar se uma letra é uma vogal, com múltiplos casos, comparando diferentes abordagens de controle de fluxo utilizando `if-else`.

- **Menu Interativo**:
  - Criação de um menu interativo que permite ao usuário selecionar opções como:
    - Cadastrar, buscar ou apagar clientes.
    - Possibilidade de encerrar o programa, utilizando controle de fluxo com `while` e `switch` para manter o menu ativo até o usuário escolher a opção de saída.

### 5. Estruturas de Repetição: `while`, `do-while` e `for`
- **Exemplo de laço `do-while`**:
  - Um laço que solicita ao usuário a entrada de números, somando-os até que o usuário digite `0`, demonstrando como esse tipo de laço funciona ao menos uma vez antes de verificar a condição.
- **Exemplo de laço `while`**:
  - Laço que multiplica números em uma tabela de multiplicação e interrompe no meio do processo usando `break`.
- **Exemplo de laço `for`**:
  - Tabela de multiplicação completa utilizando o laço `for`, executando uma operação repetitiva controlada por um contador.

### 6. Conversão de Tipos de Dados (Casting)
...
### 7. Manipulação de Datas com `DateTime`
...
### 8. Trabalhando com Strings e Interpolação
...
### 9. Orientação a Objetos: Criação de Classes e Métodos
...
### 10. Controle de Estoque e Simulação de Vendas
